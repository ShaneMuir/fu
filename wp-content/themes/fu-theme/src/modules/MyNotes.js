...
class MyNotes {
    ...
    async deleteNote(e) {
        ...
        try {
            const response = await axios.delete(fuData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"))
    ...
    async updateNote(e) {
        ...
        try {
            const response = await axios.post(fuData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost)
            this.makeNoteReadOnly(thisNote)
        } catch (e) {
            console.log("Sorry")
        }
    ...
    async createNote() {
        ...
        if(document.querySelector('.new-note-title').value == "") {
            alert('Title is a req field!')
        } else {
            try {
